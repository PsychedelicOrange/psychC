...
unsigned int create_shader(char *vertexPath, char *fragPath) {
  ...
  unsigned int vertexShader =
      compile_shader(vertexShaderCode, GL_VERTEX_SHADER);
  unsigned int fragmentShader =
      compile_shader(fragmentShaderCode, GL_FRAGMENT_SHADER);
...
unsigned int create_program(unsigned int vertexShader,
  ...
  if (!success) {
    ...
    loge("failed to link shaders: %s", infoLog);
    return -1;
  }
...
/* This might cause slowness ?? can delegate shaderProgram selection to user
 * maybe */
...
void setUniformFloat3(unsigned int shaderProgram, float *floatArray,
                      char *location) {
  glUseProgram(shaderProgram);
  int loc = glGetUniformLocation(shaderProgram, location);
  glUniform3fv(loc, 1, floatArray);
}
void setUniformFloat4(unsigned int shaderProgram, float *floatArray,
                      char *location) {
  glUseProgram(shaderProgram);
  int loc = glGetUniformLocation(shaderProgram, location);
