...
static void cursor_position_callback(GLFWwindow *window, double xpos,
                                     double ypos);
void gl_settings() { glEnable(GL_DEPTH_TEST); }
...
int main(int argc, char *argv[]) {
  ...
  init_glfw(4, 3);
  GLFWwindow *window =
      (GLFWwindow *)create_glfw_window(800, 600, "psychspiration\0");
  lastX = 800.0f / 2;
  lastY = 600.0f / 2;
  ...
  unsigned int environmentShader =
      create_shader("/home/parth/code/psychC/shaders/env.vs",
                    "/home/parth/code/psychC/shaders/env.fs");
  unsigned int actorShader =
      create_shader("/home/parth/code/psychC/shaders/actor.vs",
                    "/home/parth/code/psychC/shaders/actor.fs");
  logd("Shaders Loaded.");

  {
    mat4s projection = glms_perspective(glm_rad(45.0f), (float)800 / (float)600,
                                        0.01f, 10000.0f);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    setUniformFloat4(environmentShader, cam.position.raw, "cam_pos");
...
static void cursor_position_callback(GLFWwindow *window, double xpos,
  ...
  float yoffset =
      ypos - lastY; // reversed since y-coordinates go from bottom to top
  lastX = xpos;
  lastY = ypos;
