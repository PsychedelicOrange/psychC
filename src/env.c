...
unsigned int load_texture(cgltf_texture *texture) {
  ...
  if (buf_view == NULL) { // image is external file
    ...
    data = stbi_load_from_memory(image.data, image.size, &width, &height,
                                 &nrChannels, 0);
    if (!data) {
  ...
  default:
    internal_format = GL_RGB;
    logw("Not supported : image has %i no. of channels. Setting format to "
         "GL_RGB.",
         nrChannels);
  }
...
material_env load_material_env(cgltf_material *material) {
  ...
  if (material->has_pbr_metallic_roughness) {
    ...
    logd("Values for factors: base_color: [%f,%f,%f,%f] metal: %f, rough: %f",
         ...
         mat.base_color_factor[2], mat.base_color_factor[3],
...
void upload_material_to_shader(unsigned int shader, material_env mat) {
  setUniformInt(shader, mat.base_color != -1, "base_color_texture_exists");
  setUniformInt(shader, mat.rough_metal != -1, "roughmetal_texture_exists");
  setUniformInt(shader, mat.normal != -1, "normal_texture_exists");
  setUniformFloat4(shader, mat.base_color_factor, "base_color_factor");
  setUniformFloat(shader, mat.metallic_factor, "metallic_factor");
  setUniformFloat(shader, mat.roughness_factor, "roughness_factor");
}

void load_primitives_env(cgltf_mesh *mesh, primitive_env *primitives,
  ...
  for (size_t i = 0; i < mpriv_count; i++) {
    ...
    if (mesh->primitives[i].material !=
        NULL) { // some meshes don't have materials
    ...
    for (size_t j = 0; j < attribute_count; j++) {
      ...
      void *att_buf =
          buf_view->buffer->data + buf_view->offset + accessor->offset;
...
void vertexAttrib_env() {
  ...
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(vertex_env),
                        (void *)(offsetof(vertex_env, position)));
  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(vertex_env),
                        (void *)(offsetof(vertex_env, normal)));
  glEnableVertexAttribArray(2);
  glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(vertex_env),
                        (void *)(offsetof(vertex_env, tangent)));
  glEnableVertexAttribArray(3);
  glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(vertex_env),
                        (void *)(offsetof(vertex_env, uv)));
  glEnableVertexAttribArray(4);
  glVertexAttribPointer(4, 2, GL_FLOAT, GL_FALSE, sizeof(vertex_env),
                        (void *)(offsetof(vertex_env, uv1)));
}
...
drawable_prim upload_single_primitive_env(primitive_env p) {
  ...
  glBufferData(GL_ARRAY_BUFFER, p.vertex_count * sizeof(vertex_env), p.vertices,
               GL_STATIC_DRAW);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               p.indices_count * sizeof(unsigned short), p.indices,
               GL_STATIC_DRAW);
  vertexAttrib_env();
  glEnableVertexAttribArray(0);
...
void draw_multiple_primitive_env(unsigned int shaderProgram,
                                 unsigned int env_vao,
                                 primitive_env *primitives,
                                 size_t primitive_count) {
  glUseProgram(shaderProgram);
  glBindVertexArray(env_vao);
  for (int i = 0; i < primitive_count; i++) {
    // logd("glDrawElementsBaseVertex(GL_TRIANGLES, %li,
    // GL_UNSIGNED_SHORT,(void*) %li, %li)" ,primitives[i].indices_count,
    // (primitives[i].base_index) , primitives[i].base_vertex);
    bind_textures(primitives[i].material);
    upload_material_to_shader(shaderProgram, primitives[i].material);
    glDrawElementsBaseVertex(
        GL_TRIANGLES, primitives[i].indices_count, GL_UNSIGNED_SHORT,
        (void *)(primitives[i].base_index * sizeof(unsigned short)),
        primitives[i].base_vertex);
  }
}

buffer append_primitive_vertice_data(primitive_env *primitives, size_t from,
                                     size_t to) {
  vertex_env *vertices;
  ...
  for (int i = from; i < to; i++) {
    memcpy(ptr, primitives[i].vertices,
           primitives[i].vertex_count * sizeof(vertex_env));
...
buffer append_primitive_indice_data(primitive_env *primitives, size_t from,
                                    size_t to) {
  ...
  for (int i = from; i < to; i++) {
    memcpy(ptr, primitives[i].indices,
           primitives[i].indices_count * sizeof(unsigned short));
...
unsigned int upload_multiple_primitive_env(primitive_env *primitives,
                                           size_t primitive_count) {
  ...
  buffer vertices =
      append_primitive_vertice_data(primitives, 0, primitive_count);
  glBindBuffer(GL_ARRAY_BUFFER, vbo);
